...
--[[
	input is a Tensor of size (seqLength+2) * batchsize * (vocabSize+1)
	seq is a LongTensor of size batchsize * seqLength.
...
function crit:updateOutput(input, seq)
	...
	for b = 1, batchsize do -- iterate over batches
		...
		for t = 2, L do -- iterate over sequence time (ignore t=1, dummy forward for the image)
			...
			local target_index
			if t-1 > seqLength then -- we are out of bounds of the index sequence: pad with null tokens
				target_index = 0
			else
				target_index = seq[{b, t-1}]
			end
