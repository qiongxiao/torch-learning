...
local SN, parent = torch.class('nn.SmalCNNet', 'nn.Module')

function SN:__init(kwargs, output_dim)
	parent.__init(self)
	...
	-- 32/2/2/2/2 = 2
	self.net:add(nn.View(512*2))

	-- stage FC 1
	if self.dropout > 0 then
		self.net:add(nn.Dropout(self.dropout))
	end
	self.net:add(nn.Linear(512*2, 512))
...
function SN:updateOutput( input )
	return self.net:forward(input)
end

function SN:backward( input, gradOutput, scale )
	return self.net:backward(input, gradOutput, scale)
end

function SN:parameters()
	return self.net:parameters()
end

function SN:training()
	self.net:training()
	parent.training(self)
end

function SN:evaluate()
	self.net:evaluate()
